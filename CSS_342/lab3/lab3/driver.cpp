...
template <typename T>
void testCopyConstructor(List<T>& byRefList, List<T> byValList) 
{
...
int main() 
{
    ...
    // list one
    mylist.insert(new NodeData(4, ' '));
    mylist.insert(new NodeData(3, 'n'));
    mylist.insert(new NodeData(15, 'f'));
    mylist.insert(new NodeData(16, 'u'));

    // list three
    mylist2.insert(new NodeData(16, 'u'));
    mylist2.insert(new NodeData(14, 't'));
    mylist2.insert(new NodeData(18, 'F'));
    mylist2.insert(new NodeData(14, 's'));
    mylist2.insert(new NodeData(25, '!'));
    mylist2.insert(new NodeData(15, 'f'));

    cout << "mylist:" << endl << mylist << endl;
    cout << "mylist2:" << endl << mylist2 << endl;

    List<NodeData> copied(mylist2);
    cout << "copied:" << endl << copied << endl;

    mylist3.intersect(mylist, mylist2);
    cout << "mylist3.intersect(mylist, mylist2);" << endl << "mylist:" << 
        endl << mylist << endl << "mylist2:" << endl << mylist2 << endl << 
        "mylist3:" << endl << mylist3 << endl << endl;

    mylist3.intersect(mylist3, mylist2);
    ...
    List<Employee> company1, company2, company3, company4, company5, company6, 
        company7, company8, company9, company10, company11, missingInputFile,
        circularOne, circularTwo;

    // for electronic version: 
    ifstream infile1("../data31.txt"), infile2("../data32.txt"),
             infile3("../data33.txt"), infile4("../data34.txt"),
             doesNotExist("../somenonexistentfile.txt");
    missingInputFile.buildList(doesNotExist);
    ...
    company5.merge(company5, company5);
    ...
    Employee empHead("AAAA", "BBBBB");

    // Pre-removal comparison
    cout << "Before alteration: company10 " << (company10 == company11 ? "equals":"does not equal") 
        << " company11" << endl;
