...
public class Kernel
{
    ...
    public final static int FSREAD   = 20; // SysLib.read(int fd, byte buffer[])
    public final static int FSWRITE  = 21; // SysLib.write(int fd, byte buffer[])
    ...
    case INTERRUPT_SOFTWARE: // System calls
        ...
        // instantiate and start a filesystem
        fs = new FileSystem(disk.diskSize);

        // Format with default size if FS is missing
        if (! fs.formatted())
        {
            fs.format(64);
        }
        ...
        case OPEN:
        {
            ...
            return fs.open(scheduler.getMyTcb(), a[0], a[1]);
        }
        
        case CLOSE:
            return fs.close(scheduler.getMyTcb(), param);
            
        case SIZE:
            return fs.fsize(scheduler.getMyTcb(), param);
            
        case SEEK:
        {
            int[] a = (int[])(args);
            return fs.seek(scheduler.getMyTcb(), param, a[0], a[1]);
        }
        ...
        case FSREAD:
            return fs.read(scheduler.getMyTcb(), param, (byte[])args);
            
        case FSWRITE:
            return fs.write(scheduler.getMyTcb(), param, (byte[])args);
        }
        return ERROR;
