...
public class SysLib {
    // sizeof() constants (CB = "count of bytes")
    public static final int CBSHORT = 2;
    public static final int CBCHAR = 1;
    public static final int CBINT = 4;
    ...
    public static synchronized int rawread( int blkNumber, byte[] b ) {
        return Kernel.interrupt( Kernel.INTERRUPT_SOFTWARE,
                 Kernel.RAWREAD, blkNumber, b );
    }

    public static synchronized int rawwrite( int blkNumber, byte[] b ) {
        return Kernel.interrupt( Kernel.INTERRUPT_SOFTWARE,
    ...
    // FileSystem interrupts
    public static int format(int maxFiles)
    {
        return Kernel.interrupt( Kernel.INTERRUPT_SOFTWARE,
                Kernel.FORMAT, maxFiles, null );
    }
    ...
    {
        return Kernel.interrupt( Kernel.INTERRUPT_SOFTWARE,
                Kernel.FSREAD, fd, buffer );
    }
    
    public static int write(int fd, byte buffer[])
    {
        return Kernel.interrupt( Kernel.INTERRUPT_SOFTWARE,
                Kernel.FSWRITE, fd, buffer );
    }
