...
/******************************************************************************
* A <CODE>ByteBTNode</CODE> provides a node for a binary tree. Each node 
...
*   <A HREF="../../../../edu/colorado/nodes/ByteBTNode.java">
*   http://www.cs.colorado.edu/~main/edu/colorado/nodes/ByteBTNode.java </A>
*
* @author Michael Main 
...
{
   // Invariant of the ByteBTNode class:
   //   1. Each node has one byte value, stored in the instance
   //      variable data.
   //   2. The instance variables left and right are references to the node's
   //      left and right children.
   private byte data;
   private ByteBTNode left, right;   

   /**
   * Initialize a <CODE>ByteBTNode</CODE> with a specified initial data and links
   ...
   **/
   public ByteBTNode removeLeftmost( )
   {
      if (left == null)
         return right;
      else
   ...
   *   this node.
   **/
   public void setRight(ByteBTNode newRight)
   {                    
      right = newRight;
   }  
   ...
   **/ 
   public static ByteBTNode treeCopy(ByteBTNode source)
   {
      ByteBTNode leftCopy, rightCopy;
      ...
      else
      {
         ...
         return new ByteBTNode(source.data, leftCopy, rightCopy);
      }
   }
   ...
   **/ 
   public static int treeSize(ByteBTNode root)
   {
      if (root == null)
         return 0;
      else
