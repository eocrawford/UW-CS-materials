...
/******************************************************************************
* A <CODE>DoubleBTNode</CODE> provides a node for a binary tree. Each node 
...
*   <A HREF="../../../../edu/colorado/nodes/DoubleBTNode.java">
*   http://www.cs.colorado.edu/~main/edu/colorado/nodes/DoubleBTNode.java </A>
*
* @author Michael Main 
...
{
   // Invariant of the DoubleBTNode class:
   //   1. Each node has one double value, stored in the instance
   //      variable data.
   //   2. The instance variables left and right are references to the node's
   //      left and right children.
   private double data;
   private DoubleBTNode left, right;   

   /**
   * Initialize a <CODE>DoubleBTNode</CODE> with a specified initial data and links
   ...
   **/   
   public DoubleBTNode(double initialData, DoubleBTNode initialLeft, DoubleBTNode initialRight)
   {
      data = initialData;
      left = initialLeft;
      right = initialRight;
   ...
   **/
   public DoubleBTNode removeLeftmost( )
   {
      if (left == null)
         return right;
      else
   ...
   *   node (since that one node has now been removed).
   **/
   public DoubleBTNode removeRightmost( )
   {
      if (right == null)
         return left;
      else
   ...
   *   this node.
   **/
   public void setLeft(DoubleBTNode newLeft)
   {                    
      left = newLeft;
   }
   ...
   *   this node.
   **/
   public void setRight(DoubleBTNode newRight)
   {                    
      right = newRight;
   }  
   ...
   **/ 
   public static DoubleBTNode treeCopy(DoubleBTNode source)
   {
      DoubleBTNode leftCopy, rightCopy;
      ...
      else
      {
         ...
         return new DoubleBTNode(source.data, leftCopy, rightCopy);
      }
   }
   ...
   **/ 
   public static int treeSize(DoubleBTNode root)
   {
      if (root == null)
         return 0;
      else
